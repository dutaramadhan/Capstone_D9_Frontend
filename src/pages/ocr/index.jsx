...
export default function OCR() {
  ...
  const [parseResult, setParseResult] = useState({
    table: [[{ value: null, isValid: false }]],
    document: {},
  });
  ...
  const handleSendData = async () => {
    ...
    const requestBody = parseResult.document.map((column) => {
      ...
      for (const key in column.readings)
        record[key.toLowerCase().replace(/[\/ ]/g, "_")] = column.readings[key]
          .isValid
          ? column.readings[key].value
          : null;
      return record;
    });
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_PHP_API}/readings.php`,
        {
          method: "POST",
          headers: {
    ...
    <Layout>
      ...
      <Card title="Pendataan Komposisi Sampah dengan Kamera">
        <div className={`w-full lg:h-60 flex flex-col lg:flex-row`}>
          <label
            htmlFor="cameraInput"
            className={`lg:w-48 px-8 py-12 shadow rounded-xl block ${
              flowState < FLOW.LOADING_ANNOTATION
          ...
          <div className="text-center flex-1 max-w-full">
            <p
              className={
                flowState === FLOW.LOADING_PREPROCESSING
                  ? "text-center"
                  : "hidden"
              }
            >
            ...
            <canvas
              ...
              className={`max-w-full mt-3 lg:mt-0 h-full rounded-xl lg:rounded mx-auto ${
                flowState >= FLOW.IMAGE_PREPROCESSED ? "" : "hidden "
              }`}
              ref={canvasRef}
            />
          </div>
        ...
        <div>
          <button
            className={`text-white w-full p-2 rounded-xl mt-2 flex items-center justify-center ${
              flowState === FLOW.LOADING_ANNOTATION
                ? "bg-gray-500"
                : "bg-gray-800 hover:bg-gray-900"
            } ${
              FLOW.IMAGE_PREPROCESSED <= flowState &&
              flowState <= FLOW.LOADING_ANNOTATION
                ? ""
                : "hidden"
            }`}
          ...
          <button
            className={`text-white w-full p-2 rounded-xl mt-2 flex items-center justify-center ${
              ...
              FLOW.IMAGE_ANNOTATED <= flowState &&
              flowState <= FLOW.LOADING_SEND_DATA
                ? ""
                : "hidden"
            }`}
      ...
      <Card
        className={`mt-4 flex flex-col align-center justify-center text-center pt-6 ${
          flowState >= FLOW.LOADING_ANNOTATION ? "" : "hidden"
        }`}
      >
        <p
          className={`${flowState === FLOW.LOADING_ANNOTATION ? "" : "hidden"}`}
        >
          Memproses Deteksi Tabel Pendataan Sampah...
        </p>
        <CategoryTable
          table={parseResult.table}
          className={flowState >= FLOW.IMAGE_ANNOTATED ? "" : "hidden"}
        />
      </Card>
      <Modal isOpen={flowState > FLOW.LOADING_SEND_DATA}>
        <img
          src={`/modal/${
            flowState === FLOW.SEND_DATA_SUCCESS
        ...
        >
          {flowState === FLOW.SEND_DATA_SUCCESS ? "BERHASIL" : "GAGAL"} MENGIRIM
          DATA
        </p>
        <Link
          href="/"
          className="block rounded-xl bg-gray-800 text-white text-lg py-1 mt-5"
        >
          Kembali
        </Link>
      </Modal>
